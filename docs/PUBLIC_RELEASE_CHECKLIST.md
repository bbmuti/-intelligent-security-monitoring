...
- [x] Private vulnerability reporting is enabled before inviting external users.
...
- [ ] Add the remaining `fastapi` repository topic; `cybersecurity`, `react`, `isolation-forest`, `siem`, `mitre-attack`, `postgresql`, and `docker` are already present.
- [x] Add the final public repository URL to the Turkish and English CV entries.

## Visibility change

The repository is public. CI run [`34206457054`](https://github.com/bbmuti/SecureOps/actions/runs/34206457054) completed successfully on `main` after the visibility change and E2E stability fix, including the secret scan, backend/PostgreSQL checks, frontend tests/build, browser E2E flow, Windows collector validation, and container builds. Private vulnerability reporting, CodeQL, secret protection, Dependabot, repository topics, and the `main` branch ruleset are enabled.
